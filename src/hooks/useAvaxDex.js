...
const useAvaxDex = (chain) => {
  const { Moralis, account } = useMoralis();
  const [tokenList, setTokenlist] = useState();


async function getHello(){
    const howdy = await Moralis.Cloud.run("Hello");
    console.log(howdy);
  ...
  useEffect(() => {
   ...
   if (!Moralis?.["Plugins"]?.["oneInch"]) return null;
    //I do not have the plug I need to make my own
    //I need cloud functions to do these things
